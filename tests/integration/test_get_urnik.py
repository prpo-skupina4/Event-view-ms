...
async def test_get_urnik_sorted_by_day_and_time(client, db_session):
    # db_session je sync Session -> brez await
    ...
    db_session.execute(delete(UrnikiDB).where(UrnikiDB.uporabnik_id == 42))
    db_session.commit()
    ...
    t2 = TerminiDB(
        predmet_id=1, zacetek=time(10, 0), dolzina=90, dan=1, lokacija="P02", tip="AV"
    )
    t1 = TerminiDB(
        predmet_id=1, zacetek=time(8, 0), dolzina=90, dan=1, lokacija="P01", tip="P"
    )
